...
- Fixed a literal-scanning flaw that let a secret survive redaction. Single- and double-quoted literals were matched in two sequential passes, so an apostrophe inside a double-quoted value (`"O'Brien"`) let the single-quote pass run past it and stop on the *opening* quote of a later literal, consuming only that quote and leaving the following value intact. `SELECT * FROM t WHERE name = "O'Brien" AND token = 'SECRET123'` redacted to `... name = "O''SECRET123'`. Both quote styles are now matched in one alternation pass, so every literal is closed by its own delimiter.
- The same flaw was present in `QueryFingerprinter`, where it mattered more: that output is stored as `debugbar_profiles.slowest_fp` and shown in the Analytics N+1 leaderboard, so an unredacted literal persisted for the whole retention window rather than for the stash's fifteen minutes. Both classes now share one pattern, since duplicating it is what allowed them to carry the same bug independently. An upgrade step clears stored fingerprints that still contain a quote character — a fully normalised fingerprint has none, so the predicate finds exactly the affected rows and leaves clean history intact.
- Extended literal normalisation to the quote-free forms. A bare `\d+` rule consumed only the leading `0` of a hexadecimal literal and left the payload standing, so `0x534543524554` passed through redaction completely untouched and reached `slowest_fp` as `?x534543524554`; binary literals behaved the same way and `6.022e23` reduced to `0e23`. Hexadecimal, binary, and exponent forms are now matched explicitly.
- Redaction now reports failure instead of returning an empty statement. PCRE's JIT reaches its stack limit on a single very long quoted literal, at which point `preg_replace()` returns null and a string cast silently produced `''`. Input is length-checked up front and every pass is checked for failure, returning a stated marker. Callers cap SQL well below the threshold, so this is a backstop rather than a live fault.
- The scan now verifies its own work and refuses when it cannot account for a statement. A regex cannot model backtick identifiers or SQL comments containing a quote character, servers running with `NO_BACKSLASH_ESCAPES`, or unterminated literals carried by a failed query — in each, a later literal could survive. The check is exact rather than a list of suspicious constructs: once every literal has been removed, no quote character may remain, and one that does is proof the scan did not model the statement. Those cases now degrade to no EXPLAIN rather than to a leak, so the guarantee is "either every literal was removed, or nothing is returned". `QueryFingerprinter` delegates to the same routine, so it refuses in exactly the same cases.
- Gated `beacon.php` and `explain.php` on the shared access policy. The beacon checked module admin and debug mode but never `debugbar_enable`, so web-vitals rows were still accepted into `debugbar_profiles` with DebugBar switched off; EXPLAIN checked neither, so it stayed reachable with the module disabled.
...
- Split the access policy in two, because one gate was being asked to answer two different questions. `AccessPolicy::isAllowed()` still guards the admin pages, which expose site-wide SQL, sessions, and configuration accumulated across other people's requests, and it requires webmaster-group membership through `xoops_isDeveloperRequest()`. A new `isRuntimeAllowed()` guards the toolbar itself and the three endpoints that serve it, using the same admin flag the preload has always used to decide whether to render the bar. The two had diverged on two axes — group membership, and `xoops_isDeveloperRequest()` accepting only debug modes 1 and 2 — so on `debug_mode = 3`, or for a module admin outside the webmaster group, the bar rendered buttons whose endpoints answered 403. Both wrappers delegate to the same pure predicate, so the three-way decision cannot drift between them.
- Grew the unit suite from four tests to over a hundred, covering the access policy and its two gates, diagnostic sanitiser, SQL redaction and query fingerprinting, Monolog log parsing and cataloguing, system diagnostics, endpoint gating, manifest/preference consistency, and profile-schema consistency between the DDL and the insert statement. Exact counts are deliberately not quoted here: they were wrong twice while this release was being prepared, and a number that has to be re-synced on every added test is a worse record than the list of what is covered.
...
- Removed the `collect_authz` preference before release. It offered an Authz tab logging group-permission checks, and nothing implemented it. It could not simply be implemented either: `XoopsGroupPermHandler::checkRight()` fires no preload event, and `xoops_getHandler()` caches handlers in a function-local static, so a module cannot decorate the handler the way the events collector decorates the preload event table. That needs a core-side seam first, and a toggle that advertises a capability it does not have is worse than no toggle. A test now asserts every declared preference is read by something.

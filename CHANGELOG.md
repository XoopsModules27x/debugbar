...
- `SqlRedactor`'s documentation no longer claims more than a regex can deliver. It now states the cases it does not cover — backtick identifiers and SQL comments containing a quote character, servers running with `NO_BACKSLASH_ESCAPES`, and unterminated literals from failed queries — each of which can let a following literal survive. Closing them requires a tokenizer rather than a pattern.

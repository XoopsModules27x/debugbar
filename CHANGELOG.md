...
- Sealed the Analytics view model. `ProfileRepository`, `XdebugStatus::read()`, and `AnalyticsBuilder::build()` returned `array<string, mixed>`, so static analysis could not check a single one of the admin page's key accesses — three key-mismatch bugs had already shipped behind that blind spot. Each producer now declares the exact keys it returns, which makes a renamed column or a typo an analysis error rather than a blank cell.

        ...
        render() {
            ...
            this.list = new ListWidget({ itemRenderer(li, value) {
                ...
                if (value.occurrences > 1) {
                    ...
                    occurrences.classList.add(csscls('occurrences'));
                    occurrences.title = 'Occurrences';
                    occurrences.textContent = `×${value.occurrences}`;
                    li.prepend(occurrences);
                }
                if (value.context && Object.keys(value.context).length > 0) {
                    ...
                    if (Object.keys(value.context).length > 0) {
                        ...
                        copy.classList.add(csscls('copy-details'));
                        copy.addEventListener('click', (event) => {
                            event.stopPropagation();
                            copyToClipboard(diagnosticText(value)).then((success) => {
                        ...
                        explain.addEventListener('click', (event) => {
                            ...
                            fetch(config.url, {
                                ...
                                try {
                                    ...
                                    data = {error: 'HTTP ' + response.status + ': ' + raw.slice(0, 500)};
                                }
            ...
            this.bindAttr(['excludelabel', 'excludecollector', 'search'], function () {
                ...
                this.get('data').forEach((item) => {
                    ...
                    if (item.context) {
                        message += ' ' + JSON.stringify(item.context);
                    }
                ...
                fdata.forEach((item) => {
                    ...
                    const key = `${item.label || ''}\u0000${item.collector || ''}\u0000${item.message || ''}\u0000${context.source || ''}`;
                    if (!grouped.has(key)) {
                        grouped.set(key, { ...item, occurrences: 1 });
                    } else {
                        grouped.get(key).occurrences++;
                    }
                });
                this.list.set('data', Array.from(grouped.values()));
            });
        }

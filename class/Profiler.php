...
final class Profiler
{
    ...
    private function __construct() { $this->requestId = bin2hex(random_bytes(8)); }
    ...
    public function finalize(DebugbarLogger $logger): void
    {
        ...
        try {
            ...
            $stats = QueryAnalyzer::analyze($queries, $logger->getSlowQueryThreshold(), (int) $budgets['nplus1_threshold']);
            ...
            $module = '';
            if (isset($GLOBALS['xoopsModule']) && is_object($GLOBALS['xoopsModule']) && method_exists($GLOBALS['xoopsModule'], 'getVar')) $module = (string) $GLOBALS['xoopsModule']->getVar('dirname', 'n');
            ...
            if (is_object($debugbar)) {
                ...
                $debugbar->addCollector(new ConfigCollector([
                    'Flags' => implode(', ', BudgetChecker::decodeFlags($verdict['flags'])) ?: 'none',
                    'Findings' => $verdict['findings'] ?: ['none'],
                    'N+1 candidates' => $stats['n_plus_one'] ?: ['none'],
                ], 'Performance'));
            }
    ...
    {
        $config = [];
        try { $config = DebugbarCoreConfig::get(); } catch (\Throwable) { }
    ...
    private function payloadKb(): float { return ob_get_level() > 0 ? strlen((string) ob_get_contents()) / 1024 : 0.0; }
    private function isFragment(): bool { return Request::getString('HTTP_X_REQUESTED_WITH', '', 'SERVER') === 'XMLHttpRequest' || Request::getString('HTTP_HX_REQUEST', '', 'SERVER') !== '' || Request::getString('HTTP_X_REQUESTED_FRAGMENT', '', 'SERVER') !== ''; }
    private function path(string $uri): string { $path = parse_url($uri, PHP_URL_PATH); return substr(is_string($path) && $path !== '' ? $path : '/', 0, 500); }

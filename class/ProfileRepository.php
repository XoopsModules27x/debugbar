...
final class ProfileRepository
{
    ...
    public function insert(array $row, int $retentionDays = 7, int $maxRows = 10000): bool
    {
        try {
            ...
            if (array_key_exists('profiles_enable', $config) && empty($config['profiles_enable'])) return false;
        } catch (\Throwable) { }
        $db = $this->connection();
        if ($db === null || !$this->exists()) return false;
        $q = static fn(string $v): string => $db->quote(substr($v, 0, 500));
        $sql = sprintf('INSERT INTO %s (request_id,created,url,url_hash,dirname,is_fragment,is_admin_side,total_ms,boot_ms,query_count,query_ms,slowest_ms,slowest_fp,n_plus_one,peak_mem_kb,payload_bytes,flags) VALUES (%s,%u,%s,%s,%s,%u,%u,%.1F,%.1F,%u,%.1F,%.1F,%s,%u,%u,%u,%u)',
            $this->table($db), $q((string) ($row['request_id'] ?? '')), (int) ($row['created'] ?? time()), $q((string) ($row['url'] ?? '')), $q((string) ($row['url_hash'] ?? '')), $q((string) ($row['dirname'] ?? '')), (int) !empty($row['is_fragment']), (int) !empty($row['is_admin_side']), (float) ($row['total_ms'] ?? 0), (float) ($row['boot_ms'] ?? 0), (int) ($row['query_count'] ?? 0), (float) ($row['query_ms'] ?? 0), (float) ($row['slowest_ms'] ?? 0), $q((string) ($row['slowest_fp'] ?? '')), (int) ($row['n_plus_one'] ?? 0), (int) ($row['peak_mem_kb'] ?? 0), (int) ($row['payload_bytes'] ?? 0), (int) ($row['flags'] ?? 0));
        try {
            $ok = (bool) $db->exec($sql);
            if ($ok && random_int(1, 25) === 1) $this->trim($retentionDays, $maxRows);
            return $ok;
        } catch (\Throwable) { return false; }
    }
    ...
    {
        $db = $this->connection();
        if ($db === null || !$this->exists()) return [];
        try {
            $result = $db->query($sql);
            if (!$db->isResultSet($result) || !($result instanceof \mysqli_result)) return [];
            $rows = [];
            while (false !== ($row = $db->fetchArray($result))) { $rows[] = $row; }
            return $rows;
    ...
    private function trim(int $days, int $maxRows): void
    {
        ...
        if ($db === null) return;
        try {
            if ($days > 0) $db->exec(sprintf('DELETE FROM %s WHERE created < %u', $this->table($db), time() - $days * 86400));
            $count = $this->count();
            if ($maxRows > 0 && $count > $maxRows) $db->exec(sprintf('DELETE FROM %s ORDER BY profile_id ASC LIMIT %u', $this->table($db), $count - $maxRows));
        } catch (\Throwable) { }
    }
}
